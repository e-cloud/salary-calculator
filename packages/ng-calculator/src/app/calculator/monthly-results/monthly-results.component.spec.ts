...
describe(
  'MonthlyResultsComponent 月度计算结果明细组件测试',
  { timeout: 20000 },
  () => {
    ...
    it('用户在月份明细中修改补充公积金比例并点击更新时，应触发 updateMeta 事件', async () => {
      // Arrange
      const user = userEvent.setup();
      const { updateMetaSpy } = await setup();
      const firstHeader = screen.getAllByText(/实收：/i)[0];
      await user.click(firstHeader);

      // Act
      const suppEmpInput = screen.getByTestId(
        'input-supplementary-housing-fund-rate-1',
      );
      const suppEmployerInput = screen.getByTestId(
        'input-supplementary-housing-fund-employer-rate-1',
      );
      const updateBtn = screen.getByTestId('btn-update-month-1');

      await user.clear(suppEmpInput);
      await user.type(suppEmpInput, '4');
      await user.clear(suppEmployerInput);
      await user.type(suppEmployerInput, '6');
      await user.click(updateBtn);

      // Assert
      expect(updateMetaSpy).toHaveBeenCalled();
      const emittedValue = updateMetaSpy.mock.calls[0][0];
      expect(emittedValue.meta.supplementaryHousingFundRate).toBe(4);
      expect(emittedValue.meta.supplementaryHousingFundEmployerRate).toBe(6);
      expect(emittedValue.index).toBe(0);
    }, 15000);

    it('用户在弹窗中确认同步时，应同时发出当前月及所有后续月份的 updateMeta 更新', async () => {
      // Arrange
      const user = userEvent.setup();
      const { updateMetaSpy } = await setup();
      mockDialog.open.mockReturnValue({
        afterClosed: () => of(true), // 模拟用户点击“确认同步”
      });
      const firstHeader = screen.getAllByText(/实收：/i)[0];
      await user.click(firstHeader);
      ...
      await user.clear(suppEmpInput);
      await user.type(suppEmpInput, '5');
      await user.click(updateBtn);

      // Assert
      // 确认同步后，第 2~12 月依次发出更新，最后发出第 1 月更新，总共发出 12 次 updateMeta 事件
      expect(updateMetaSpy).toHaveBeenCalledTimes(12);
    }, 15000);
  },
);
